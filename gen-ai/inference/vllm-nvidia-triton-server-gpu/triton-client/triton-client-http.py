...
model_endpoint = os.getenv("MODEL_ENDPOINT", "/v2/models/mistral7b/generate")
service_name = os.getenv("SERVICE_NAME", "http://localhost:53550")
...
async def generate_text(session, prompt):
    ...
    inputs.append(httpclient.InferInput("STREAM", [1], "BOOL"))
    ...
    if True:
        ...
        inputs.append(httpclient.InferInput("SAMPLING_PARAMETERS", [1], "BYTES"))
        inputs[-1].set_data_from_numpy(sampling_parameters_data)

    outputs = []
    outputs.append(httpclient.InferRequestedOutput("TEXT"))
    payload = {
        "PROMPT": "<s>[INST]" + SYSTEM_PROMPT + prompt + "[/INST]",
        "STREAM": False,
        "SAMPLING_PARAMETERS": json.dumps(sampling_parameters),  # Increased to allow for longer responses
    }
