...
Scope: standard EKS + gVisor runtime tier + chained Cilium egress. A reference Python agent lives under `../../blueprints/agents/sandbox-demo/` — see that directory's README for how to exercise the blueprint end-to-end.
...
- An IAM role with `bedrock:InvokeModel` + trust policy allowing EKS Pod Identity — templates at `manifests/iam-bedrock-trust-policy.template.json` and `manifests/iam-bedrock-permissions.template.json`.
...
See `../../blueprints/agents/sandbox-demo/README.md` for interactive-run instructions and troubleshooting.
...
└── manifests/
    ...
    ├── demo-agent.yaml                         # Direct Sandbox resource for the reference agent
    ├── rgd-agent-sandbox.yaml                  # KRO ResourceGraphDefinition — composes Sandbox + SA + policy
    ├── agent-sandbox-demo-instance.yaml        # AgentSandbox instance via the KRO RGD (optional)
